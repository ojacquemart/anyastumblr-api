...
import tumblr._
import tumblr.dao.{JoiesDuCodeConfiguration, HfrConfiguration}
...
class SiteConfigurationsSpec extends Specification {
  ...
  "The SiteConfigurations class" should {
    ...
    "build tumblr configuration" in {
      ...
      config.navigationAscending must be equalTo(false)
    ...
    "build joiesducode configuration" in {
      ...
      config.navigationAscending must be equalTo(true)
